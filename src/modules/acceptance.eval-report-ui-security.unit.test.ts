...
// H-13. Several tests here shell out to `pnpm eval` / `pnpm report` or ingest a corpus, which is
// integration-shaped work sitting in the `unit` project, and that project takes vitest's 5s default.
// They measured ~2.1s idle, ~2.3x of margin, and that margin is not enough on a loaded machine or a
// shared CI runner: on 2026-07-16 a clean `origin/main` worktree failed 4, then 5, then 3 of these 9
// tests across three consecutive runs of untouched code, every failure `Test timed out in 5000ms`,
// with a fourth run green. A required check that fails on load does not just break builds — it
// launders real regressions as noise, and noise as regressions. It also blocked an unrelated commit
// twice through the pre-commit hook.
//
// The timeout is the whole fix: nothing here is actually slow, it is subprocess start-up under
// contention. The alternative was moving the file into the `integration` project, which would have
// quietly dropped it out of `check:fast`.
const subprocessTimeoutMs = 30_000;

// No mocks: eval parsing and scoring run through the production JSONL parser.
it(
  "fails malformed golden sets and computes thresholded machine-readable scores",
  async () => {
...
it(
  "fails the pnpm eval command on an empty golden set",
  async () => {
...
it(
  "generates deterministic Article 50 bundles with window filtering",
  async () => {
...
it(
  "writes Typst PDF, JSON, and sealed audit excerpt through the pnpm report command",
  async () => {
...
it(
  "renders German console, source, report, CSP, citations, and no analytics",
  async () => {
    const app = createReportWindowApp();
    await app.ingest.ingest({ corpusDir: "examples/eu-ai-act" });
    const session = app.bootstrapOperator("operator@example.local");
...
it(
  "serves console, replay, report, and download routes with self-only CSP",
  async () => {
