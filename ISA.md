...
- [ ] ISC-4: Operator console German UI ships with all error messages, button labels, audit panels, and report-generation copy in `de-DE`; `Accept-Language` parsing exists but `de-DE` is the only fully translated locale in v1.
- [ ] ISC-5: Anti: `/auth/operator` does not accept passwords; password fields do not exist in the database schema.
- [ ] ISC-6: Anti: No anonymous query path. `GET /api/query` without a session returns `401`.
...
- [ ] ISC-49: lefthook fast gate runs at every commit; pre-push runs integration; CI runs `pnpm check:full` on every push.
