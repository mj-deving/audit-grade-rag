...
- **License:** Apache 2.0. Permits self-hosting, modification and redistribution. The copyright holder is declared in `NOTICE`, which Apache-2.0 section 4(d) requires redistributions to carry forward.
...
- **2026-07-15 — Apache 2.0 over BSL 1.1. Supersedes the 2026-05-10 license decision.** BSL was dropped. Commit `4761d71` adopted Apache 2.0 across `LICENSE`, `package.json`, the README and the demo footer, resolving a three-way inconsistency in which `package.json` said UNLICENSED, the demo footer and the project registry said BSL 1.1, and no `LICENSE` file existed at all. The four-year change date and the hosted-SaaS restriction it carried no longer apply. The 2026-05-10 entries stay as the record of what was decided then.
